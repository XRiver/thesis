...
日志结构合并树（Log-Structured Merge-tree，下文称LSM树）是基于磁盘的数据结构，旨在在长期运行过程中在文件上提供记录的高频率插入和删除，以及低成本索引~\cite{DBLP:conf/IEEEcloud/SongYH18}。LSM树使用延迟和批量索引更改的算法，将更改从内存中的组件通过一个或多个磁盘组件进行级联~\cite{DBLP:conf/socc/MittalN19}。这种高效的方式类似于到合并排序。在此过程中，所有索引值都可以通过内存组件或磁盘组件连续访问。相比B树等传统访问方法，该算法大大减少了磁盘臂的移动~\cite{DBLP:conf/apweb/ZhuHQZLZ17}。LSM树还可以推广到其他操作，比如插入和删除~\cite{DBLP:journals/acta/ONeilCGO96}。
